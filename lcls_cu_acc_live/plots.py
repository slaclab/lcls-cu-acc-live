...
data = {'x': [0, 0, 0, 0], 'y': [0, 0, 0, 0], "name": [0, 0, 0, 0]}
...
twiss.line(
    x="x", y="y", source=b_beta_cds, line_width=1, name=r'$\beta_b$', legend_label="βy"
)
...
title_div = Div(
    text=f"<h1>LUME LCLS cu_hxr model</h1>",
...
table_variables = [
    ...
    if input_var.variable_type == "scalar"
]
...
curdoc().add_root(
    column(
        title_div,
        row(column(twiss, e_total, eta_etap), value_table.table),
